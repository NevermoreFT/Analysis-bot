...
def parse_message(text):
    ...
    if 'итоговый остаток' in text_lower or 'остаток наличных' in text_lower:
        ...
        if amount_match:
            ...
            return {'type': 'balance', 'amount': amount, 'raw': text}
    ...
    if '✅' in text or 'пополнение' in text_lower:
        ...
        if amount_match:
            amount = float(amount_match.group(1).replace('.', '').replace(',', ''))
            return {'type': 'income', 'amount': amount, 'raw': text}
    if '❌' in text or 'изъяти' in text_lower or 'выдано' in text_lower or 'запрос на изъятия' in text_lower:
        ...
        if amount_match:
            amount = float(amount_match.group(1).replace('.', '').replace(',', ''))
            return {'type': 'expense', 'amount': amount, 'raw': text}
...
def generate_report(user_id):
    ...
    for msg in messages:
        ...
        if parsed:
            if parsed['type'] == 'income':
                income_messages.append(msg); income_total += parsed['amount']
            elif parsed['type'] == 'expense':
                expense_messages.append(msg); expense_total += parsed['amount']
            elif parsed['type'] == 'start': start_message = msg
            elif parsed['type'] == 'end': end_message = msg
    ...
    for msg in income_messages: report += msg + "\n"
    ...
    for msg in expense_messages: report += msg + "\n"
    if end_message:
        report += "\n" + end_message
    else:
